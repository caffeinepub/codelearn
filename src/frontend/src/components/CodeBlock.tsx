    ...
    if (/[a-zA-Z_]/.test(code[i])) {
      ...
      if (KEYWORDS.has(word)) {
        ...
        const isAfterDef =
          ...
          secondLast.value === "def";
        tokens.push({
          type: isAfterDef ? "function" : "plain",
          value: word,
          id: nextId(),
        });
      }
    ...
    <pre className="bg-code p-5 overflow-x-auto m-0">
      <code className="font-mono text-sm leading-relaxed">
        {tokens.map((token) => {
          ...
          if (!cls) {
            return (
              <React.Fragment key={token.id}>{token.value}</React.Fragment>
            );
          }
          return (
